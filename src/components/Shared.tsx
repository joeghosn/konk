...
export const SectionTitle: React.FC<{eyebrow?: string, title: string, subtitle?: string}> = ({eyebrow, title, subtitle}) => (
  <div className="text-center max-w-3xl mx-auto mb-10">
    {eyebrow && <p className="text-xs tracking-widest uppercase text-kaspa-400/80">{eyebrow}</p>}
    <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight drop-shadow-[0_0_20px_rgba(31,215,187,0.18)]">{title}</h2>
    {subtitle && <p className="mt-3 text-base sm:text-lg text-white/70">{subtitle}</p>}
  </div>
)

export const GlowButton: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({className='', children, ...props}) => (
  <button {...props}
